...
export default function CartScreen() {
  ...
  const subtotal = useMemo(
    () =>
      cartItems.reduce(
        (sum: number, it: any) =>
          sum + (it.discounted_price ?? it.retail_price ?? it.price ?? 0) * (it.quantity ?? 1),
        0
      ),
    [cartItems]
  );
  ...
  const onIncrease = (id: string, currentQty: number = 1) => {
    const newQty = currentQty + 1;
    dispatch(updateQuantity({ id, quantity: newQty } as any));
  };
  ...
  const onCheckout = async () => {
    ...
    try {
      if (typeof sessionStorage !== "undefined") token = sessionStorage.getItem("authToken");
        ...
        // native fallback
        try {
          // require at runtime to avoid bundling issues if package missing
                  ...
                  <View style={styles.itemBody}>
                    ...
                    <View style={styles.priceRow}>
                      ...
                      <View style={styles.qtyRow}>
                        <TouchableOpacity onPress={() => onDecrease(item._id, item.quantity ?? 1)} style={styles.qtyBtn}>
                          <Ionicons name="remove" size={16} color="#333" />
                        </TouchableOpacity>
                        <Text style={styles.qtyText}>{item.quantity ?? 1}</Text>
                        <TouchableOpacity onPress={() => onIncrease(item._id, item.quantity ?? 1)} style={styles.qtyBtn}>
